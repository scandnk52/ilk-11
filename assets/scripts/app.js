class App {
  ...
  changeLog = [
    ...
    {
      v: "v3.1",
      logs: ["Sayfa boyutu değiştirme sonrası kayma problemi düzeltildi."],
    },
    {
      v: "v3.0",
      logs: [
        "Oyuncu sürükleme eklendi.",
        "Takım ve oyuncu bilgileri değiştirme menüsü eklendi.",
        "Resimlerde sürükle bırak ve panodan direkt yapıştırma eklendi.",
        "Arayüz butonları değiştirildi.",
        "Mobil uyumlu tasarım eklendi.",
        "Varsayılan resimler düzenlendi.",
        "Arayüz tekrar yenilendi.",
      ],
    },
    {
      v: "v2.21",
      logs: ["Takım gizleme sonucu kayma sorun düzeltildi."],
    },
    {
      v: "v2.2",
      logs: [
        "Takım gizleme özelliği eklendi.",
        "Oyuncu resimlerinin bozulması düzeltildi.",
      ],
    },
    {
      v: "v2.1",
      logs: ["Oyuncuların kayması düzeltildi."],
    },
    {
      v: "v2.0",
      logs: [
        "Arayüz yenilendi.",
        "Diziliş sistemi değiştirildi.",
        "Varsayılan resimler değiştirildi.",
      ],
    },
    {
      v: "v1.1",
      logs: ["Oyuncuların tekrar oluşması düzeltildi."],
    },
    {
      v: "v1.0",
      logs: ["Tam sürüm yayınlandı.", "Formasyon seçme sorunu düzeltildi."],
    },
    {
      v: "v0.9",
      logs: ["Hata düzeltildi."],
    },
  ];
  ...
  buildFabs() {
    ...
    newsFab.onclick = this.news.bind(this);
  ...
  settings() {
    ...
    const checkboxFirstTeam = this.checkBoxInput(
      "1. Takımı gizle",
      this.teams[0].hide,
      this.teams[1].hide
    );
    const checkboxSecondTeam = this.checkBoxInput(
      "2. Takımı gizle",
      this.teams[1].hide,
      this.teams[0].hide
    );
    const checkboxFullSize = this.checkBoxInput(
      "Diğer takım gizlenince takımı tüm sahaya diz",
      this.fullSize
    );
    const checkboxRememberPlayers = this.checkBoxInput(
      "Oyuncu adlarını hatırla",
      this.rememberPlayers
    );
  ...
  news() {
    ...
    newsDiv.classList.add("changelog");

    const nameDiv = document.createElement("h1");
    nameDiv.classList.add("app-name");
    nameDiv.innerHTML = this.appName + " " + this.version;

    newsDiv.append(nameDiv);

    for (let i = 0; i < this.changeLog.length; i++) {
      const versionElement = document.createElement("ul");
      versionElement.classList.add("version");

      const vElement = document.createElement("h3");
      vElement.innerText = this.changeLog[i].v;

      versionElement.append(vElement);

      for (let j = 0; j < this.changeLog[i].logs.length; j++) {
        const logElement = document.createElement("li");

        logElement.innerText = this.changeLog[i].logs[j];

        versionElement.append(logElement);
      }

      newsDiv.append(versionElement);
    }

    this.createModal("Yenilikler", newsDiv, true);
  }
  ...
  buildFormations(team) {
    ...
    for (let i = 0; i < this.formations.length; i++) {
      ...
      for (let j = 0; j < this.formations[i].lineups.length; j++) {
        ...
        team.formation[0] == i && team.formation[1] == j
    ...
    const teamDetailsElement =
      team.element.getElementsByClassName("team-details")[0];
  ...
  buildPlayers(team) {
    ...
    for (
      let i = 0;
      i <
      this.formations[team.formation[0]].lineups[team.formation[1]].lineup
        .length;
      i++
    ) {
  ...
  lineUpTeam(team) {
    ...
    for (let i = 0; i < players.length; i++) {
      const reverseHeight =
        this.fullSize && (this.teams[0].hide || this.teams[1].hide)
          ? 0
          : !this.fullSize && team.reverse && hasHide
          ? 0
          : team.reverse
          ? this.height / 2
          : 0;
      const position = team.reverse
        ? this.parcel +
          1 -
          this.formations[formation[0]].lineups[formation[1]].lineup[i][0]
        : this.formations[formation[0]].lineups[formation[1]].lineup[i][0];
      const height =
        this.fullSize && (this.teams[0].hide || this.teams[1].hide)
          ? this.height
      ...
      players[i].style.top =
        (height / (this.parcel + 1)) * position -
        players[i].offsetHeight / 2 +
        reverseHeight +
        "px";
      players[i].style.left =
        (this.width / (this.parcel + 1)) *
          this.formations[formation[0]].lineups[formation[1]].lineup[i][1] -
  ...
  createModal(title, element, noCancel) {
    ...
    modalCloseHeaderElement.classList.add(
      "modal-button",
      "modal-circle-button",
      "modal-close"
    );
